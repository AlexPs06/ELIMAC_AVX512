...
void H_128(__m128i * nonce,  __m128i *key, unsigned rounds,unsigned nblks);
void I_128(__m128i * nonce,  __m128i  key, unsigned rounds,unsigned nblks);
...
// }
 ...
 char infoString[]= "EliMAC AVX512";  /* Each AE implementation must have a global one */
...
void ELIMAC(unsigned char *K_1, unsigned char *K_2, unsigned char *M, int size, unsigned char *T){
    ...
    int m_blocks_128=size/16;
    static __m512i * plain_text_512 = (__m512i*) M;
    ...
    for (i = 0; i < m_blocks-1; i++){
        ...
        nonce_temp[0]=nonce; 
        H(nonce_temp,  keys_512, 7, pipeline);
    ...
    for (size_t j = 0; j < 4; j++){
        Tag=_mm_xor_si128(Tag,S.bl128[j]);
    }

    int final_blocks = m_blocks_128-block_process-1;

    i = i*4;
    static __m128i * plain_text_128 = (__m128i*) M;
    S.bl512=nonce;
    nonce_128 = S.bl128[0];

    while (final_blocks){
        nonce_temp_128[0] = nonce_128;

        
        H_128(nonce_temp_128,  keys_128, 7, pipeline);
        
        plain_text_128[i]=_mm_xor_si128(plain_text_128[i],nonce_temp_128[0]);

        I_128(&plain_text_128[i],  keys_0_128, 4,pipeline);

        Tag=_mm_xor_si128(plain_text_128[i],Tag);
        nonce_128=_mm_add_epi64(nonce_128, sum_nonce_128);
        i++;
        final_blocks--;
    ...
    Tag=_mm_xor_si128(Tag,plain_text_128[i]);
